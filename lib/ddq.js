    ...
    class Ddq extends EventEmitter {
        ...
        close(callback) {
            ...
            if (!this.busy && this.backend.connection) {
                ...
                this.done(new Error("Could not close. DeDuplicated Queue is either already busy or the connection is already closed."));
            }
        }
        ...
        decrementAndDone(err) {
            console.log("DecrementAndDone called");
         ...
         */
            ...
            if (err) {
                console.log("AN ERROR OCCURRED AND WAS SENT TO DONE:");
                console.log(err);
        ...
        listenStart(callback) {
            ...
            if (!this.busy && this.backend.connection && !this.isListening) {
                ...
                this.done(new Error("Could not start listening. DeDuplicated Queue is either busy, does not have an open connection, or is already listening."));
            }
        }
        ...
        open(callback) {
            ...
            if (!this.busy && !this.backend.connection) {
                ...
                this.done(new Error("Could not open. DeDuplicated Queue is either busy or the connection to the database is already open."));
            }
        }
        ...
        sendMessage(message, callback, errorCallback, topic) {
            ...
            if (!this.busy && this.backend.connection) {
                ...
                this.done(new Error("Could not send message. DeDuplicated Queue is either busy or the connection to the database is not open."));
            }
        }
    }
